...
public class FileReaderDecodeXML<T> {

	@SuppressWarnings("unchecked")
	public List<T> getLista(StreamFactory factory, File file, String nameReader) throws Exception {
		List<T> lista = new ArrayList<>();
		try (
			FileInputStream fileInputStream = new FileInputStream(file);
			InputStreamReader inputStreamReader = new InputStreamReader(fileInputStream, StandardCharsets.UTF_8)) {
        
			BeanReader beanReader = factory.createReader(nameReader, inputStreamReader); 
	        Object record = null;
